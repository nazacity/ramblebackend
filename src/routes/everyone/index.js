...
const confirmPayment = async (req, res) => {
  ...
  const updatedUserActivity = await model.UserActivity.findByIdAndUpdate(
    ...
    .populate({ path: 'user', select: { device_token: 1, lineId: 1 } })
    .populate({
      path: 'activity.id',
...
const testLineMsg = async (req, res) => {
  try {
    const data = JSON.stringify({
      to: 'U83584e6690b2d22b4a604ac227348d9a',
      messages: [
        {
          type: 'flex',
          altText: `ยืนยันการชำระเงิน`,
...
router.get('/testLine', testLineMsg);
